...
项目没有把 `/models` 做成账号池推断结果，也没有做复杂的本地聚合。

当前原则是：

- `/models` 仍然通过正常上游路径获取真实快照
- 会在本地缓存最近一次成功的上游 models 快照
- Codex 调用方始终直接走上游刷新当前快照
- 对非 Codex 调用方：
  - 如果缓存仍在 TTL 内，直接返回 fresh snapshot
  - 如果只有过期缓存，先返回当前 snapshot，并在后台触发一次刷新
  - 如果还没有缓存，则同步请求上游
- 不为了“看起来聪明”去拼装跨账号聚合结果或发明额外可见性语义

原因：

- 这条路径需要一份稳定的 models 快照，用来给其他接口推导默认的 `parallel_tool_calls`
- 上游本来就知道哪些模型可见，网关不应重建一套模型可见性判定
- 保留轻量缓存能降低 `/models` 访问抖动，但不至于把项目推向复杂状态机
